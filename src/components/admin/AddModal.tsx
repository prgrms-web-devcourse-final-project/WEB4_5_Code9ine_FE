...
export default function AddModal({
  ...
  const submitHandler = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!name.trim()) return toast.error('갓플 이름을 입력해야 합니다.');
    if (!address.trim()) return toast.error('갓플 주소를 입력해야 합니다.');
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
        <form
          className="w-[500px] rounded-[10px] bg-[var(--white-color)] p-[20px] shadow-[var(--shadow-md)]"
        ...
        >
          ...
          <button
            type="submit"
            className={`cursor-pointer rounded-[20px] bg-[var(--main-color-2)] px-[10px] text-[#2b2e34]`}
          >
            등록
          </button>
        </form>
