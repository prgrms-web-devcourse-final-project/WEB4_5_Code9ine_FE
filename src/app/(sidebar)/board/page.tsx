...
import PostItemSkeleton from '@/components/board/PostItemSkeleton';
...
export default function Page() {
  ...
  const popularPostSelect = async (postId: number) => {
    try {
      setLoadingpoPopularPost(true);
      const post = await boardApi.getPostById(postId);
      setSinglePost(post);
    } catch (err) {
      console.error('인기글 단건 조회 실패:', err);
    } finally {
      setLoadingpoPopularPost(false);
    }
  };
  ...
  return (
    <div className="flex w-full max-w-screen flex-col px-[15px] select-none md:flex-row md:px-[0px]">
      <div className="hide-scrollbar order-1 mt-[15px] ml-0 flex max-h-[310px] flex-1 flex-col overflow-y-auto rounded-[10px] shadow md:order-2 md:mt-0 md:ml-[15px] md:h-[869px] md:max-h-none md:w-[350px]">
        <PopularPostList
          onSelectPost={popularPostSelect}
          ...
          <>
            ...
            {loadingMore &&
              Array.from({ length: 1 }).map((_, i) => (
                <div key={'more-' + i} className="mb-[15px]">
                  <PostItemSkeleton />
                </div>
              ))}
